...
function createObj(data){
    ...
    const newobj = new WeatherObj(data.current.temp_f,data.current.temp_c,data.current.condition.text,data.location.name,data.location.region,
        ...
        data.current.vis_miles,data.current.vis_km,date[0]);
    console.log(newobj);
    ...
    for(let i=1;i<data.forecast.forecastday.length;i++) {
        const weekforecast = new ForecastObj(data.forecast.forecastday[i].day.maxtemp_c, data.forecast.forecastday[i].day.maxtemp_f,
            data.forecast.forecastday[i].day.mintemp_c, data.forecast.forecastday[i].day.mintemp_f,data.forecast.forecastday[i].day.condition.text, data.forecast.forecastday[i].date);
...
function forecastDomManip(newobj){
    ...
    newobj._sevenDay.forEach(element => {
        ...
        const weatherPicBottom = document.createElement('div');
        weatherPicBottom.textContent=element.condition;
